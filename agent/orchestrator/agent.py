...
def execute(curl_command: str) -> Dict:
    """
    ...
    Returns:
        Dict: A dictionary containing the execution result with keys:
            - success (bool): Whether the command executed successfully
            - output (str): The command output if successful
            - error (str): Error message if the command failed
    """
    print(f"--- Tool: execute called with command: {curl_command} ---")
    ...
    try:
        # Parse the curl command safely using shlex
...
attack_agent = Agent(
    ...
    For each endpoint, you will execute the attack using the payloads generated by the Payload Agent.
        give a curl command for each payload and Your task is to execute the curl requests generated by the Attack Agent. "
    ''',
    tools=[execute])
...
strategy_agent= Agent(
    name="strategy_agent",
    model="gemini-2.0-flash-exp",
    description="You are the Strategy Agent. Your task is to execute the curl requests generated by the Attack Agent. ",
    tools=[execute]
)
